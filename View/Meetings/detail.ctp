...
<form action="/gachakoi/meetings/confirm" enctype="multipart/form-data" role="form" id="MeetingConfirmForm" method="post" accept-charset="utf-8">
    ...
    <label for="MeetingDateMonth">デート日</label>

    <select name="data[Meeting][date][year]" class="form-control" id="MeetingDateYear">
    <?php for($year=date('Y'); $year<=date("Y",strtotime("+1 year")); $year++) {
        print('<option value="'.$year.'">'.$year.'</option>');
    } ?>
    </select>

    <select name="data[Meeting][date][month]" class="form-control" id="MeetingDateMonth">
      <?php 
        $this_month=date('n');
    ...
    <select name="data[Meeting][date][day]" class="form-control" id="MeetingDateDay">
     <?php 
        $today=date('d');
